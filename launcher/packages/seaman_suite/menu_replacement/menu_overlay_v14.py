...
BUTTON_LAYOUT = [
    ...
    {'id': 93, 'img': 'embedded_048_36x36.png'}
    ]
...
class MenuOverlay:
    def __init__(self, fullscreen=False):
        ...
        self.root = tk.Tk()
        self.root.title("Seaman Menu Overlay v14" + (" (fullscreen)" if fullscreen else " (windowed)"))
        ...
        user32.GetWindowRect(self.game_hwnd, ctypes.byref(rect))
        if fullscreen:
            self.game_w = self.root.winfo_screenwidth()
            self.game_h = self.root.winfo_screenheight()
            self.game_x, self.game_y = 0, 0
            # menu cluster centred on the game window's actual screen position
            menu_cx = (rect.left + rect.right) // 2
            menu_cy = (rect.top + rect.bottom) // 2 - 140
        else:
            self.game_x, self.game_y = rect.left, rect.top
            self.game_w, self.game_h = rect.right - rect.left, rect.bottom - rect.top
            menu_cx = self.game_w // 2
            menu_cy = self.game_h // 2 - 140
        ...
        self.root.update(); self.apply_ct(); self.center_x, self.center_y = menu_cx, menu_cy
    ...
    def create_ui(self):
        ...
        self.canvas.tag_bind('drag', '<ButtonPress-1>', self.on_ds)
        self.canvas.tag_bind('drag', '<B1-Motion>', self.on_dm)
        self.root.bind('<B1-Motion>', self.on_dm)
    ...
    def on_ds(self, e): self.dsx, self.dsy = e.x_root, e.y_root; self._dragging = True
    def on_dm(self, e):
        if not self._dragging: return
        dx, dy = e.x_root - self.dsx, e.y_root - self.dsy
        if self.fullscreen:
            # move canvas items and game window; overlay stays fullscreen
            self.canvas.move('all', dx, dy)
            self.center_x += dx; self.center_y += dy
            rect = ctypes.wintypes.RECT()
            user32.GetWindowRect(self.game_hwnd, ctypes.byref(rect))
            user32.SetWindowPos(self.game_hwnd, None, rect.left + dx, rect.top + dy, 0, 0, 0x05)
        else:
            self.game_x += dx; self.game_y += dy
            self.root.geometry(f"+{self.game_x}+{self.game_y}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fullscreen', action='store_true', help='Fullscreen mode — no title bar on game window')
    args = parser.parse_args()
    MenuOverlay(fullscreen=args.fullscreen).run()
